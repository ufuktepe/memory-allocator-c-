...
sub run_sh61 ($;%) {
    ...
    eval {
        do {
            ...
            if (defined($size_limit) && $size_limit_file && @$size_limit_file) {
                ...
                if ($len > $size_limit) {
                    $died = "output p_file size $len, expected <= $size_limit";
                    die "!";
                }
            }
    ...
    if (exists($answer->{p_status})
        && ($answer->{p_status} & 127) == $SIGINT
...
sub print_killed ($$) {
    my($p_file, $out) = @_;
    printf STDERR "${Red}\r${p_file} FAIL: Killed after %.5fs: $out->{killed}$Off\n", $out->{time};
...
if ($Exec) {
    ...
    if ($Make) {
        ...
        if ($out && $out->{p_status} == 0 && exists($out->{output})) {
            foreach my $tn (@tests) {
